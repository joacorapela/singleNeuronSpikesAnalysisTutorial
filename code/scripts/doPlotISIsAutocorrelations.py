...
def main(argv):
    ...
    parser.add_argument("--end_time_sec",
                        ...
                        default=180.0)
    parser.add_argument("--bin_size_msec",
                        type=int,
                        help="ISIs bin size (msec)",
                        default="1")
    ...
    female1_ISIs[(female1_ISIs==0).nonzero()[0]] = 1.0
    ...
    female2_ISIs[(female2_ISIs==0).nonzero()[0]] = 1.0

    female1_autocorrelations = timeSeries.autocorr(female1_ISIs, num_lags)
    female1_significance = 2.0/np.sqrt(len(female1_ISIs))

    female2_autocorrelations = timeSeries.autocorr(female2_ISIs, num_lags)
    female2_significance = 2.0/np.sqrt(len(female2_ISIs))

    diff_autocorrelations = female1_autocorrelations-female2_autocorrelations
    diff_std = np.sqrt(1/len(female1_ISIs)+1/len(female2_ISIs))
    diff_significance = 2*diff_std

    lags_msec = np.arange(num_lags)*bin_size_msec
